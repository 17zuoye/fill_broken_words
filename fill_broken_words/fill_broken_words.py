...
class FillBrokenWordsClass(SentenceProcess):
    ...
    def fix_params_strs(self, env):
            ...
            if env.split_block_group.original_fillblank_length < env.params_strs.original_len:
                ...
                if env.split_block_group.original_fillblank_length != 0:
                    ...
                    for size1 in split_size:
                        ...
                        if env.inspect:
                            print "[strs_order_combinations]", strs_order_combinations
        ...
        if env.inspect:
            print "[params_strs:%i] %s ..." % (len(env.params_strs), env.params_strs[0:10])
        env.params_strs[:] = list(set(env.params_strs))
        ...
        if env.min_groups_len > 1:
            env.min_groups_len -= 1
    ...
    def generate_possible_patterns_with_strs_list(self, env):
        ...
        if env.inspect:
            print "before [possible_patterns_groups]", possible_patterns_groups
        for idx1, g1 in enumerate(possible_patterns_groups):
            ...
            for i1 in g1:
                ...
                if i1.count(None) == env.split_block_group.original_fillblank_length:
                    new_possible_patterns_groups.append([i1])
        ...
        if env.inspect:
            print "after [possible_patterns_groups]", possible_patterns_groups
    ...
    def generate_candidate_patterns_vs_word_groups_s(self, env):
        ...
        if env.inspect:
            print "[env.possible_patterns_with_strs_list]", len(env.possible_patterns_with_strs_list)
...
# 1440 个，所以可能慢, 其他都不到100, 甚至个位数
        ...
        if env.inspect:
            ...
            print "*" * 30
            for i1 in candidate_patterns_vs_word_groups:
                print i1
            print "*" * 30
            print
...
# TODO slow!!!
        ...
        if env.inspect:
            print "[candidate_patterns_vs_word_groups_s] %i items" % len(env.candidate_patterns_vs_word_groups_s)
...
# TODO slow!!!
        ...
        for candidate_patterns_vs_word_groups1 in env.candidate_patterns_vs_word_groups_s:
            if env.inspect:
                print "[candidate_patterns_vs_word_groups1]", candidate_patterns_vs_word_groups1
            if env.inspect:
                print
            ...
            if env.params_strs.has_merged_at_least_one and (env.split_block_group.original_fillblank_length == 1) and (len(candidate_patterns_vs_word_groups1[0].patterns) == 1):
                if env.inspect:
                    print "[env.sentence]", env.sentence
